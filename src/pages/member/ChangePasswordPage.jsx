...
export function ChangePasswordPage() {
  ...
  const inAppShell = location.pathname.startsWith('/member') || location.pathname.startsWith('/mentor')
  const needsReset = !!user?.user_metadata?.force_password_reset
  const canSubmit = useMemo(() => pw.length >= 10 && pw === pw2 && !busy, [pw, pw2, busy])

  const description = needsReset
  ...
  if (inAppShell) {
    return (
      <WorkspacePage>
        <WorkspaceHeader label="Security" title="Change your password" description={description} />
        <WorkspacePanel>{form}</WorkspacePanel>
      </WorkspacePage>
    )
  }
