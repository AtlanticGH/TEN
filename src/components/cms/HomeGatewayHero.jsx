    ...
    >
      ...
      <div className="relative z-10 mx-auto max-w-7xl px-6 pb-28 pt-36 sm:px-8 md:pb-32 md:pt-40 lg:px-10">
        <div className="max-w-4xl">
          ...
          <h1 className="text-[clamp(1.875rem,4.5vw,3.5rem)] font-black leading-[1.12] tracking-tight text-white">
            {c.headline_before ? <span className="block pb-0.5">{c.headline_before}</span> : null}
            {c.headline_emphasis ? (
              <span className="block bg-clip-text pb-1 text-transparent" style={{ ...ORANGE_GRADIENT, WebkitBackgroundClip: 'text' }}>
                {c.headline_emphasis}
              </span>
            ) : (
              c.headline ? <span className="block">{c.headline}</span> : null
            )}
          </h1>
